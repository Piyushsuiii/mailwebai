...
import { shouldProcessSync } from "@/lib/sync-dedup";
...
export const POST = async (req: NextRequest) => {
    ...
    // Deduplicate: prevent the same webhook from triggering multiple syncs
    // across multiple instances or webhook retries
    const eventId = payload.payloads?.[0]?.id || timestamp;
    const shouldProcess = await shouldProcessSync(
        payload.accountId.toString(),
        eventId
    );
    if (!shouldProcess) {
        console.log(`[Webhook] Duplicate sync for account ${payload.accountId}, skipping`);
