...
import { invalidateThreadCaches } from './email-cache';
...
async function syncEmailsToDatabase(emails: EmailMessage[], accountId: string) {
    ...
    try {
        ...
        async function syncToOrama() {
            await Promise.all(emails.map(email => {
                return limit(async () => {
                    ...
                    console.log(`Inserted email ${email.subject} into Orama`)
                })
            }))
        }
        ...
        // Bust thread caches after sync completes
        await invalidateThreadCaches(accountId);
    } catch (error) {
